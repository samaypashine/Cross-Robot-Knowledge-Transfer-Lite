...
from paper5.utils import time_taken, get_config, get_classes_labels, get_classifier, classifier, augment_trials, \
    update_all_modalities, compute_mean_accuracy, save_config, get_split_data_objects, split_train_test_objects, \
    split_train_test_trials, get_new_labels, check_kema_data, plot_fold_all_modalities_v2, plot_each_modality, \
    plot_all_modalities_v2
...
if __name__ == '__main__':
   ...
   For data augmentation (-augment-trials), augment trials of the test objects in source and target contexts.
    ...
    For images, color histogram of last image in the video is used.
    '''

    np.random.seed(0)

    parser = argparse.ArgumentParser(description='across tools, behaviors, tools_behaviors knowledge transfer')
    ...
    parser.add_argument('-across',
                        choices=['tools', 'behaviors', 'tools_behaviors', 'robots'],
                        default='robots',
    ...
    '''
    test_percentage = 0.34
    folds_objects_split = split_train_test_objects(args.num_folds, list(objects), test_percentage)
    save_config(folds_objects_split, 'configs' + os.sep + 'objects_splits.yaml')
    '''
    ...
    # num_objects_list = [1, len(folds_objects_split['fold_0']['train'])]
                    ...
                    for target_behavior in sorted(behaviors):
                        for target_tool in sorted(tools):
                            ...
                            if (source_behavior == target_behavior) and (source_tool == target_tool):
                                if args.across == 'robots' and source_robot != target_robot:
                                ...
                                if args.across == 'tools' and source_behavior == target_behavior and source_tool != target_tool:
                                    ...
                                    projections.append({'source_behavior': source_behavior, 'source_tool': source_tool,
                                                        'target_behavior': target_behavior, 'target_tool': target_tool})
    logging.info('Total projections: {}'.format(len(projections)))
    logging.info('Projections: {}'.format(projections))
    for p_i, projection in enumerate(projections):
        ...
        projection_path = results_path + '_'.join([source_robot, source_behavior, source_tool, 'TO',
                                                   target_robot, target_behavior, target_tool]) + '_' + args.classifier_name + os.sep
        ...
        for object_fold in sorted(folds_objects_split):
            ...
            for num_obj in num_objects_list:
                ...
                for modality in modalities:
                    ...
                    t_train_data, t_train_y = get_split_data_objects(binary_dataset_path, trials, classes_labels,
                                                                     target_robot, target_behavior, modality, target_tool,
                                                                     folds_objects_split[object_fold]['train'][0:num_obj],
                                                                     args.feature, 'resnet18')
                    ...
                    t_test_data, t_test_y = get_split_data_objects(binary_dataset_path, trials, classes_labels,
                                                                   target_robot, target_behavior, modality, target_tool,
                                                                   folds_objects_split[object_fold]['test'], args.feature, 'resnet18')
                    ...
                    s_data, s_y = get_split_data_objects(binary_dataset_path, trials, classes_labels, source_robot,
                                                         source_behavior, modality, source_tool, objects_list,
                                                         args.feature, 'resnet18')
                    ...
                    # s_test_data, s_test_y = get_split_data_objects(binary_dataset_path, trials, classes_labels,
                    #                                                source_robot, source_behavior, modality, source_tool,
                    #                                                folds_objects_split[object_fold]['test'], args.feature, 'resnet18')
                    ...
                    # Randomly choosing a trial fold for training and testing model (baselines and transfer)
                    ...
                    # s_train_data2 = s_test_data2[:, folds_trials_split[trial_fold]['train']].resize(
                    #     (-1, t_test_data2_.shape[-1]))
                    ...
                    end_time = time.time()
                    file.write('\n\n' + '_'.join([source_robot, source_behavior, source_tool, 'TO', target_robot, target_behavior, target_tool])
            ...
            file_path = results_path + '_'.join([source_robot, source_behavior, source_tool, 'TO',
                                                 target_robot, target_behavior, target_tool]) + '_' + \
                                  args.classifier_name + os.sep + object_fold + os.sep
            
            plot_fold_all_modalities_v2(folds_proba_score_bl[object_fold],
        ...
        with open(projection_path + os.sep + 'results_' + all_modalities_type + '.csv', 'w') as f:
            writer = csv.writer(f, lineterminator='\n')
            writer.writerow(['Number of Shared Objects', 'Baseline 1 Accuracy', 'Baseline 1 SD',
                             'Transfer Accuracy', 'Transfer SD',
        ...
        plot_all_modalities_v2(folds_proba_score_bl, folds_proba_score_kt, 'all_modalities',
                            title_name, xlabel, projection_path, 'all_modalities')
        plot_all_modalities_v2(folds_proba_score_bl, folds_proba_score_kt, 'all_modalities_train',
                            title_name, xlabel, projection_path, 'all_modalities_train')
        plot_all_modalities_v2(folds_proba_score_bl, folds_proba_score_kt, 'all_modalities_test',
