...
if __name__ == '__main__':
            ...
            for target_behavior in sorted(behaviors):
                for target_tool in sorted(tools):
                    ...
                    for projection in projections:
                        ...
                        if (source_behavior_ == target_behavior) and (source_tool_ == target_tool):
                            exists = True
                            break
    ...
    for p_i, projection in enumerate(projections):
        ...
        source_behavior_, source_tool_, target_behavior_, target_tool_ = fix_names([source_behavior, source_tool,
                                                                                    target_behavior, target_tool])
        projection_dir = '_'.join([source_behavior_, source_tool_, 'and', target_behavior_, target_tool_])
        ...
        for modality in modalities:
            ...
            s_data, s_y = get_split_data_objects(binary_dataset_path, trials, objects_labels, args.robot,
                                                 source_behavior, modality, source_tool, objects_list, args.feature)
            logging.info('s_data: {}'.format(s_data.shape))
            logging.info('s_y: {}, {}'.format(s_y.shape, s_y.flatten()[0:15]))

            # Get target train objects data
            t_data, t_y = get_split_data_objects(binary_dataset_path, trials, objects_labels, args.robot,
                                                 target_behavior, modality, target_tool, objects_list, args.feature)
            ...
            if Z1.shape[1] > 1:
                ...
                title1 = source_behavior_ + '-' + source_tool_ + '-' + modality_
                # For paper:
                ...
                plot_features_IE_v2(Z1, s_y, Z2, t_y, source_behavior, source_tool, target_behavior, target_tool,
